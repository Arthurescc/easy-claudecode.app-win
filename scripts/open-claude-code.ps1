...
if (-not (Test-Healthy $env:CLAUDE_ROUTER_HEALTH_URL)) {
    Start-Process $ShellBin -ArgumentList @("-NoProfile","-ExecutionPolicy","Bypass","-File",(Join-Path $PSScriptRoot "start-claude-code-router.ps1")) -WindowStyle Hidden -RedirectStandardOutput $RouterLog -RedirectStandardError $RouterLog | Out-Null
    Start-Sleep -Seconds 1
}

if (-not (Test-Healthy $env:CLAUDE_PROXY_HEALTH_URL)) {
    Start-Process $ShellBin -ArgumentList @("-NoProfile","-ExecutionPolicy","Bypass","-File",(Join-Path $PSScriptRoot "start-claude-code-dashscope-proxy.ps1")) -WindowStyle Hidden -RedirectStandardOutput $ProxyLog -RedirectStandardError $ProxyLog | Out-Null
    Start-Sleep -Seconds 1
}

if (-not (Test-Healthy $StatusUrl)) {
    Start-Process $ShellBin -ArgumentList @("-NoProfile","-ExecutionPolicy","Bypass","-File",(Join-Path $PSScriptRoot "run-claude-console.ps1")) -WindowStyle Hidden -RedirectStandardOutput $BackendLog -RedirectStandardError $BackendLog | Out-Null
}
